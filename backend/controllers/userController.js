...
exports.createUser = async (req, res) => {
  try {
    const { employeeId, name, email, password, floor, role, language } =
      req.body;
...
exports.resignUser = async (req, res) => {
  try {
    ...
    const resignDate = moment(date).tz("Asia/Ho_Chi_Minh").startOf("day");
    ...
    for (const order of orders) {
      ...
      order.days.forEach((day) => {
        const orderDate = moment(day.date)
          .tz("Asia/Ho_Chi_Minh")
          .startOf("day");

        if (orderDate.isSameOrAfter(resignDate)) {
          day.mains = [];
          day.drink = null;
          day.soup = null;
          day.dessert = null;
    ...
    return res.json({
      success: true,
      message: "Đã cập nhật nghỉ việc thành công.",
    });
  } catch (err) {
    console.log(err);
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        const language =
          row.Language?.toString().trim().toLowerCase() === "ko" ? "ko" : "vi";
        ...
        if (req.user.role !== "admin_floor") {
          const roles = ["guest", "admin_eocmn", "admin_nexon", "admin_floor"];

          role = roles.includes(row.Role) ? row.Role : "guest";
        }
