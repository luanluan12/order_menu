...
function Checkin() {
  ...
  const scanFrame = async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || t("checkin_failed"));
      ...
      {success ? (
        <div className="mx-auto mt-6 max-w-lg rounded-[36px] bg-white px-8 py-12 text-center shadow-2xl">
          <div className="mx-auto flex h-32 w-32 items-center justify-center rounded-full bg-orange-500">
            <Check size={72} strokeWidth={3.5} className="text-white" />
          </div>

          <div className="mt-8 rounded-2xl bg-gray-50 p-6 text-left">
            <div className="mb-3">
              <span className="font-semibold">{t("meal_date")}:</span>{" "}
              {new Date(checkinData.date).toLocaleDateString()}
            </div>

            <div className="mb-4">
              <span className="font-semibold">{t("received_time")}:</span>{" "}
              {new Date(checkinData.receivedAt).toLocaleString()}
            </div>

            <div className="font-semibold mb-2">{t("meal_list")}</div>

            <ul className="space-y-2">
              {checkinData.mains.map((m, i) => (
                <li key={i}>
                  {m.name}
              ...
              {checkinData.drink && <li>{checkinData.drink.name}</li>}

              {checkinData.soup && <li>{checkinData.soup.name}</li>}
            </ul>
          </div>

          <p className="mt-5 text-2xl text-gray-500">
            {t("checkin_saveScreen")}
          </p>
