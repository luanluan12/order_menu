...
import { getInvoiceReport, exportInvoiceReport } from "../../api/reportApi";
...
function InvoiceReport() {
  ...
  const formatInput = (date) => date.toISOString().split("T")[0];

  const [from, setFrom] = useState(formatInput(monday));

  const [to, setTo] = useState(formatInput(friday));
  ...
  const handleExport = async () => {
    try {
      ...
      link.download = `Invoice_Report_${from}_${to}.xlsx`;
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl bg-white p-5 shadow">
        <div className="grid gap-4 md:grid-cols-4">
          <div>
            <label className="mb-2 block text-sm font-semibold">Từ ngày</label>
          ...
          <div>
            <label className="mb-2 block text-sm font-semibold">Đến ngày</label>
      ...
      <div className="space-y-4 lg:hidden">
        {rows.length === 0 ? (
          ...
          rows.map((item, index) => (
            <div key={index} className="rounded-2xl bg-white p-5 shadow">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div>
                  <span className="font-semibold">Công ty:</span> {item.company}
                </div>

                <div>
                  <span className="font-semibold">Món ăn:</span> {item.food}
                </div>
              </div>
            </div>
          ))
      ...
      <div className="rounded-2xl bg-white p-6 shadow">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {Object.keys(summary).length === 0 ? (
            ...
            Object.entries(summary).map(([company, total]) => (
              <div
                key={company}
                className="rounded-2xl border border-orange-100 bg-orange-50 p-5"
              >
                <div className="text-sm font-medium text-gray-500">Công ty</div>
                ...
                <div className="mt-5 flex items-end justify-between">
                  <span className="text-sm text-gray-500">Tổng suất ăn</span>
